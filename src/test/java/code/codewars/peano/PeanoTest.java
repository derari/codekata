...
class PeanoTest {
    ...
    void divMod() {
        ...
        assertEquals(dm(3, 1), n(10).divMod(n(3)));
    }

    @Test
    void divMod_performance() {
        var a = n(10000);
        var b = n(100);
        Peano.DivMod result = null;

        var start = System.currentTimeMillis();
        for (int i = 0; i < 25; i++) {
            result = a.divMod(b).orElseThrow();
        }
        var end = System.currentTimeMillis();
        System.out.println("divMod_performance: " + (end - start) + "ms");

        assertEquals(b, result.div());
        assertEquals(zero(), result.mod());
    }

    private static Peano n(int n) {
        var p = zero();
        while (n-- > 0) p = p.inc();
        return p;
    }

    private static Peano.DivMod dm(int d, int m) {
        return new Peano.DivMod(n(d), n(m));
    }

    private static void assertEquals(int expected, Peano actual) {
        Assertions.assertEquals(expected, actual.asInt());
    }

    private static void assertEquals(int expected, Optional<Peano> actual) {
        assertEquals(expected, actual.orElseThrow());
    }

    private static void assertEquals(Peano.DivMod expected, Optional<Peano.DivMod> actual) {
        assertEquals(expected, actual.orElseThrow());
    }
